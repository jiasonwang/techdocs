...
.scheduleLaunchActivity.当application创建的消息发送出去以后，就该开始创建Activity并且调用它的生命周期了。在ApplicationThread（一个binder）的scheduleLaunchActivity发送一条LAUNCH_ACTIVITY消息给主线程，这样职责就