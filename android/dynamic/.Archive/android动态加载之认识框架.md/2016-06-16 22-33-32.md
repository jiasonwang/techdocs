...
![创建application]()
> 20.ActivityThread.handleLaunchActivity,ActivityThread.performLaunchActivity,Instrumentation.newActivity.通过handleLaunchActivity这个方法名字就可以看出来，接下去就是要开启act的过程，无非就是创建一个act实例，然后调用onCreate，onStart, onResume，当然中间可能会根据情况调用onRestoreInstanceState这个方法将上次存储的信息