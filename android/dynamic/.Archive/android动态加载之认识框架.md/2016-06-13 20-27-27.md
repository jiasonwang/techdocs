...
若我们没有设置特定的application，那么就会默认构造Application对象。而且，顺带会调用Application.attatch(Context)，传入的context就是ContextImpl实例。这个调用结束后，就开始进行Application生命周期的调用，通过Instrumentation的callApplicationOnCreate