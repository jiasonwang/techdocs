...
```
> 14.ActivityManagerService.startProcessLocked, 这个方法有多个重载方法，这里先调用第一个输入参数是进程名字的方法，然后构造一个ProcessRecord对象，然后再调用第一个输入参数ProcessRecord的方法中。然后是通过Process.start，来构造一个进程，改方法会fork Zygote进程来达到创建app进程的目的，而每个